...
const BgVideoForm = () => {
  ...
  const fetchTeam = async () => {
    try {
      const response = await axios.get("https://localhost:8000/bgvideoform");
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm(formData)) {
      try {
        if (editMode) {
          await axios.put(
            `https://localhost:8000/bgvideoform/${editingId}`,
            formData
          );
          toast.success("Data Updated Successfully");
        } else {
          await axios.post("https://localhost:8000/bgvideoform", formData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://localhost:8000/bgvideoform/${id}`);
