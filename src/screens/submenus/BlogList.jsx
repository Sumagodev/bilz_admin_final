...
const BlogList = () => {
  ...
  const fetchTeam = async () => {
    try {
      const response = await axios.get("https://localhost:8000/bloglist");
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm(formData)) {
      try {
        if (editMode) {
          await axios.put(
            `https://localhost:8000/bloglist/${editingId}`,
            formData
          );
          toast.success("Data Updated Successfully");
        } else {
          await axios.post("https://localhost:8000bloglist", formData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://localhost:8000/bloglist/${id}`);
